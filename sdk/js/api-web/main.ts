        ...
        if (msg.data.loaded) {
            ...
            try {
                msg.data.port.postMessage({
                    ok: sessionStorage.getItem(msg.data.key),
                });
            } catch (e) {
            ...
            try {
                msg.data.port.postMessage({
                    ok: localStorage.getItem(msg.data.key),
                });
            } catch (e) {
                msg.data.port.postMessage({ error: e.message });
                console.error(e);
...
async function call_sdk(method: string, args?: any) {
    ...
    if (callback_idx !== undefined) {
        ...
        myWorker.postMessage(
            { method, streamed: true, args: new_args, port: port2 },
            [port2]
        );
        ...
        if (path[0] === "get_bowser") {
            ...
            let new_callback = (event) => {
                if (event.V0.State?.graph?.triples) {
                    let json_str = new TextDecoder().decode(
                        event.V0.State.graph.triples
                    );
                    event.V0.State.graph.triples = JSON.parse(json_str);
                } else if (event.V0.Patch?.graph) {
                    let inserts_json_str = new TextDecoder().decode(
                        event.V0.Patch.graph.inserts
                    );
                    event.V0.Patch.graph.inserts = JSON.parse(inserts_json_str);
                    let removes_json_str = new TextDecoder().decode(
                        event.V0.Patch.graph.removes
                    );
